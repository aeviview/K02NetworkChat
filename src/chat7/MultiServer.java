...
public class MultiServer 
{
	...
	public MultiServer()
	{
		...
		Collections.synchronizedMap(clientMap); //동기화설정. 쓰레드가 사용자정보에 동시 접근하는것을 차단!
	}
	
	//채팅 서버 초기화하는 init()메소드!
	public void init()
	{
		try
		{
		...
		while(it.hasNext())
		{
			try
			{
				...
				else
				{
					...
					if(name.equals(""))
					{
						it_out.println(msg); //접속, 퇴장에서 사용되는 부분
					}
					else
					{
						it_out.println("["+ name + "] : "+ msg); //메세지를 보낼 때 사용되는 부분
					}
				}
				/*
				 클라이언트에게 메세지를 전달한다.
				 매개변수로 name이 있는 경우와 없는 경우를 구분해서 메세지를 전달한다.
				 */
			}
			catch (Exception e)
			{
	...
	class MultiServerT extends Thread
	{
		...
		public void run()
		{
			...
			try
			{
				name = in.readLine(); //클라이언트의 이름을 읽어와서 저장한다
				
				
				//방금 접속한 클라이언트를 제외한 나머지에게 사용자의 입장을 알려준다.
				sendAllMsg("", name + "님이 입장하셨습니다.", "All");
				
				clientMap.put(name,  out); //현재 접속자의 정보를 HashMap에 저장한다.
				
				System.out.println(name + " 접속"); //HashMap에 저장된 객체의 수로 접속자 수를 파악할 수 있다.
				...
				while (in!=null)
				{
					...
					if(s.charAt(0)=='/') //만약 /로 시작한다면 명령어이다!
					{
						/*
						 귓속말은 아래와 같이 전송하게 된다.
						 => /to 대화명 대화내용 : 따라서 split()을 통해 space로 문자열을 분리한다.
						 */
						String[] strArr = s.split(" ");
						/*
						 대화내용에 스페이스가 있는 경우 문장의 끝까지 출력해야 하므로
						...
						if(strArr[0].equals("/to"))
						{
							...
							//함수 호출 시 One이면 한 명에게만 메세지 전송한다!
						}
						
					}
					else //만약 /로 시작하지 않으면 일반 메세지이다!
					{
						sendAllMsg(name, s, "All"); //함수 호출 시 All이면 전체 전송한다!
					}
				}
			}	
			catch (Exception e)
			...
			finally
			{
				...
				sendAllMsg("", name + "님이 퇴장하셨습니다!", "All"); //퇴장하는 클라이언트의 쓰레드명을 보여준다.
